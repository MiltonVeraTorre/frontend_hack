    ...
    switch (error?.response?.status ?? error?.status) {
        case 400:
          Swal.fire({
              ...
              text: error?.response?.data?.msg ?? "La solicitud no pudo ser procesada, revisa los datos e intenta nuevamente"
          })
          break
        ...
        case 401:
          Swal.fire({
              icon: "error",
              title: "No autorizado",
              text: error?.response?.data?.msg ?? "No tienes autorización para realizar esta operación, inicia sesión nuevamente"
          })
          break
        ...
        case 403:
          Swal.fire({
              icon: "error",
              title: "Prohibido",
              text: error?.response?.data?.msg ?? "No tienes permiso para acceder a este recurso, contacta al administrador del sistema"
          })
          break 
        ...
        case 404:
          Swal.fire({
              ...
              text: error?.response?.data?.msg ?? "No se encontro el recurso, intenta recargar la pagina"
          })
          break
        ...
        case 500:
          Swal.fire({
              ...
              text: error?.response?.data?.msg ?? "Ha ocurrido un error de nuestro lado, reportalo lo antes posible",
          })
          break
